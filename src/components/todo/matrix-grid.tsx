...
export function MatrixGrid({ todos, onUpdateTodo, onDeleteTodo, loading = false }: MatrixGridProps) {
  ...
  return (
    <div className="w-full h-full p-2 sm:p-4">
      ...
      <div className="relative">
        ...
        {/* Mobile Axis Labels */}
        <div className="sm:hidden mb-4" role="img" aria-label="Matrix axis labels showing urgency and importance">
          <div className="flex justify-between items-center text-xs text-gray-500 mb-2">
            <span aria-label="Low urgency tasks">Low Urgency</span>
            <span className="font-medium">URGENCY</span>
            <span aria-label="High urgency tasks">High Urgency</span>
          </div>
          <div className="text-center mb-2">
            <span className="text-xs font-medium text-gray-500">IMPORTANCE</span>
          </div>
          <div className="flex justify-between text-xs text-gray-500">
            <span aria-label="High importance tasks">High ↑</span>
            <span aria-label="Low importance tasks">Low ↓</span>
          </div>
        </div>

        {/* 2x2 Grid - Responsive */}
        <div 
          className="grid grid-cols-1 sm:grid-cols-2 gap-3 sm:gap-4 min-h-[400px] sm:min-h-[450px] max-h-[80vh]"
          role="grid"
          aria-label="Eisenhower Matrix with four quadrants for task prioritization"
        >
          {loading ? (
            // Mobile-optimized skeleton loading for matrix quadrants
            <>
              {Array.from({ length: 4 }).map((_, i) => {
                const quadrantNames = ['Do First', 'Schedule', 'Delegate', 'Do Later'];
                const colors = ['bg-red-50 border-red-200', 'bg-green-50 border-green-200', 'bg-yellow-50 border-yellow-200', 'bg-gray-50 border-gray-200'];
                
                return (
                  <div
                    key={i}
                    role="gridcell"
                    aria-label={`Loading ${quadrantNames[i]} quadrant`}
                    className={`${colors[i]} rounded-xl sm:rounded-2xl border-2 p-3 sm:p-4 min-h-[180px] sm:min-h-[200px]`}
                  >
                    {/* Quadrant Header Skeleton */}
                    <div className="mb-3 sm:mb-4">
                      <div className="h-5 sm:h-6 bg-gray-300 rounded-md mb-2 animate-pulse" style={{width: '70%'}} />
                      <div className="h-3 sm:h-4 bg-gray-200 rounded-md animate-pulse" style={{width: '90%'}} />
                    </div>
                    
                    {/* Task Skeleton Items */}
                    <div className="space-y-2 sm:space-y-3">
                      {Array.from({ length: 2 }).map((_, taskIndex) => (
                        <div key={taskIndex} className="bg-white/70 rounded-lg p-2 sm:p-3 border border-gray-200/50">
                          <SkeletonLoader 
                            lines={1} 
                            showAvatar={false}
                            className="mb-2"
                          ...
                          <div className="flex justify-between items-center mt-2">
                            <div className="h-4 w-10 sm:w-12 bg-gray-200 rounded-full animate-pulse" />
                            <div className="flex gap-1">
                              <div className="h-6 w-6 sm:h-7 sm:w-7 bg-gray-200 rounded-md animate-pulse" />
                              <div className="h-6 w-6 sm:h-7 sm:w-7 bg-gray-200 rounded-md animate-pulse" />
                            </div>
                          </div>
                        </div>
            ...
            <>
              ...
              <div
                role="gridcell"
                aria-label="Do First quadrant, position 1 of 4"
              >
                <QuadrantCard
                  quadrant="do_first"
              ...
              <div
                role="gridcell"
                aria-label="Schedule quadrant, position 2 of 4"
              >
                <QuadrantCard
                  quadrant="schedule"
              ...
              <div
                role="gridcell"
                aria-label="Delegate quadrant, position 3 of 4"
              >
                <QuadrantCard
                  quadrant="delegate"
              ...
              <div
                role="gridcell"
                aria-label="Do Later quadrant, position 4 of 4"
              >
                <QuadrantCard
                  quadrant="do_later"
